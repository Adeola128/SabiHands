...
const OrgOnboarding: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const hasSeenOnboarding = localStorage.getItem('ralvo_org_onboarding_dismissed');
  ...
  const dismiss = () => {
    setIsVisible(false);
    localStorage.setItem('ralvo_org_onboarding_dismissed', 'true');
  };
      ...
      {!isMinimized && (
        <div style={{ padding: '16px' }}>
          <p style={{ margin: '0 0 16px 0', fontSize: '13px', color: '#64748B' }}>
            Welcome to Ralvo! We are excited to help you find passionate volunteers. Here are your next steps:
          </p>
          ...
          {allDone && (
            <div style={{ marginTop: '20px', padding: '12px', backgroundColor: '#F0FDF4', borderRadius: '8px', border: '1px solid #BBF7D0', color: '#166534', fontSize: '13px', fontWeight: 600, textAlign: 'center' }}>
              You're all set! Check out the settings page to verify your account. 🎉
            </div>
          )}
        </div>
      )}
