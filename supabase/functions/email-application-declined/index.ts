...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { volunteer_email, volunteer_name, gig_title, org_name, browse_gigs_url } = payload;

    if (!volunteer_email || !gig_title || !org_name) {
      throw new Error("Missing required fields in payload");
    }

    const subject = `Update on your application to ${gig_title}`;
...
<title>Update on your application to ${gig_title}</title>
<!--[if mso]>
...
<table role="presentation" class="email-container" width="600" cellpadding="0" cellspacing="0" style="width:600px; max-width:600px; background:#FFFFFF; border-radius:16px; overflow:hidden;">
  ...
  <tr><td style="background:#AFA9EC; height:6px; line-height:6px; font-size:6px;">&nbsp;</td></tr>
  ...
  <tr>
    <td class="stack-pad" align="left" style="padding:26px 48px 0;">
      <table role="presentation" cellpadding="0" cellspacing="0">
        <tr>
          <td style="border-radius:999px; background:#534AB7;">
            <a href="${displayUrl}" style="display:inline-block; padding:14px 30px; font-family:Arial,Helvetica,sans-serif; font-size:15px; font-weight:bold; color:#FFFFFF; text-decoration:none; border-radius:999px;">Browse more gigs</a>
          </td>
        </tr>
      </table>
    </td>
